...
# helper function
function ppc(u0s, ρ, σ, L_I, L_D1; K=1000, nitr_noise=100)
    @printf(SB_logfile, "Testing ρ = %.2f, σ = %.4f...\n", ρ, σ)
    @printf(ROC_logfile, "Testing ρ = %.2f, σ = %.4f...\n", ρ, σ)
    ...
    Xmat = vcat([L_I*X for X in clean_Xs]...)
    T, n = size(Xmat)

    # compute time derivatives
    clean_Ymat = f_kuramoto_3rd(Xmat, p...)
    ...
    for itr in 1:nitr_noise
        ...
        Ymat = clean_Ymat + σ*randn(size(Xmat))
...
for (k, σ) in enumerate(σs)
    for (j, ρ) in enumerate(ρs)
        pval, auc, auc3 = ppc(u0s, ρ, σ, L_I, L_D1; K=1000, nitr_noise=nitr)
        pvals[:,j,k] = pval
        aucs[:,j,k] = auc
        auc3s[:,j,k] = auc3
    end
...
writedlm("out/ppc-traj-pvals-$(n_ics)-gaussian.txt", pvals)
writedlm("out/ppc-traj-aucs-$(n_ics)-gaussian.txt", aucs)
writedlm("out/ppc-traj-auc3s-$(n_ics)-gaussian.txt", auc3s)