...
fig = Figure(size=(3.35inch, 4.75inch), fontsize=10pt)
axs = [Axis(fig[row, col], xlabel="Prop. library size", xlabelpadding=2.0, ylabel="n", limits=(nothing, nothing), xticks=(2:2:length(ρ_array), string.(ρ_array[2:2:end])), yticks=(1:length(n_array), string.(n_array)), xticklabelrotation=pi/8) for row in 2:5, col in 2:3]
...
heatmap!(axs[3,1], 1:length(ρ_array), 1:length(n_array), tri_auc, colormap=:cmr_ember, colorrange=crange_tri_auc)
heatmap!(axs[3,2], 1:length(ρ_array), 1:length(n_array), this_tri_auc, colormap=:cmr_ember, colorrange=crange_tri_auc)
...
heatmap!(axs[4,2], 1:length(ρ_array), 1:length(n_array), mean_tri_diff, colormap=:cmr_prinsenvlag, colorrange=crange_tri)
...
Colorbar(fig[3,4], label="Mean difference", colormap=:cmr_prinsenvlag, colorrange=crange)
...
Colorbar(fig[5,4], label="Mean difference", colormap=:cmr_prinsenvlag, colorrange=crange_tri)

rowsize!(fig.layout, 2, Aspect(2, 0.66))
rowsize!(fig.layout, 3, Aspect(3, 0.66))
rowsize!(fig.layout, 4, Aspect(2, 0.66))
rowsize!(fig.layout, 5, Aspect(3, 0.66))
...
text!(axs[1,1], 0.76, 0.95, text="(a)", space=:relative, font=:bold, align=(:left, :top))
text!(axs[1,2], 0.76, 0.95, text="(b)", space=:relative, font=:bold, align=(:left, :top))
text!(axs[2,2], 0.76, 0.95, text="(c)", space=:relative, font=:bold, align=(:left, :top))
text!(axs[3,1], 0.76, 0.95, text="(d)", space=:relative, font=:bold, align=(:left, :top))
